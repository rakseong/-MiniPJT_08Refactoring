	...
	<c:forEach var="purchase" items="${list}">
	 ...
	 <tr class="ct_list_pop">
	 	...
	 	<c:if test="${tranCode eq '0' || tranCode eq '1'}">
	 		<a href="/prch/getPurchase?tranNo=${purchase.tranNo}">${i}</a>
	 	</c:if>
		...
		<td align="left">
		<c:if test="${tranCode eq 2}">
			<a href="/prch/updateTranCode?tranNo=${purchase.tranNo}&tranCode=3">물건도착</a>
		</c:if>
		<c:if test="${tranCode eq 3}">
			<a href="/comment/addCommentView?tranNo=${purchase.tranNo}&prodNo=${purchase.purchaseProd.prodNo}">리뷰하기</a>
		</c:if>
		</td>
	</tr>
	<tr>
